...
Run from inside task1/ (paths like results/ and ./data/subset_ids.json are
relative), e.g.:
    python3 scripts/run_task1.py --config configs/config.yaml --step all
    python3 scripts/run_task1.py --config configs/config.yaml --step clean_baseline

The sys.path shim below makes the task1.* imports work regardless of where
...
def step_color_bias(cfg):
    ...
    for intervention_name, pil_fn in interventions.items():
        ...
        for name in tqdm(BACKBONE_NAMES, desc=f"backbones ({intervention_name})"):
            # FIX: the original loop used `clean_logits` and `logits` without
            # ever defining them. Load the backbone + trained head, get logits
            # on the intervened images, and load the cached clean logits.
            backbone = get_backbone(name, cfg)
            head = _load_head(name, backbone, len(class_names))
            clean_logits = _load_tensor(
                os.path.join(FEATURES_DIR, f"{name}_clean_test_logits.pt")
            )
...
def step_shape_vs_texture(cfg):
    """
    Requires make_cue_conflicts.py to have already been run
    ...
    if not os.path.exists(meta_path):
        raise FileNotFoundError(
            f"{meta_path} not found. Run make_cue_conflicts.py before this step."
        )
    ...
    images_tensor = torch.stack([
        to_tensor(resize(Image.open(m["path"]).convert("RGB")))
...
def step_representation_analysis(cfg):
    ...
    gray_images = _load_tensor(os.path.join(FEATURES_DIR, "grayscale_test_images.pt"))
    shuffled_images = _load_tensor(os.path.join(FEATURES_DIR, "patch_shuffle_test_images.pt"))
    ...
    cue_content_images = torch.stack([
        to_tensor(resize(train_dataset[m["content_idx"]][0].convert("RGB")))
        for m in cue_meta
    ])

    stability_results = {}

    for name in tqdm(BACKBONE_NAMES, desc="Representation analysis", unit="backbone"):
        backbone = get_backbone(name, cfg)

        # Reuse clean features from the earlier clean-baseline step
        clean_feats = _load_tensor(
            os.path.join(FEATURES_DIR, f"{name}_clean_test_features.pt")
        )

        condition_feats = {}

        # Grayscale
        condition_feats["grayscale"] = _extract_features_batched(backbone, gray_images)

        # Patch shuffle
        condition_feats["patch_shuffle"] = _extract_features_batched(backbone, shuffled_images)
        ...
        for direction in tqdm(
            ...
            images = _get_pil_images(
                test_dataset,
                test_idx,
                pil_transform=lambda img, o=offset, d=direction: T.translate(
                    img, o, d, cfg["translation"]["padding_mode"]
                ),
            )
            direction_feats.append(
                _extract_features_batched(backbone, _pil_batch_to_tensor(images))
            )

        condition_feats["translation"] = torch.stack(direction_feats).mean(dim=0)

        # Cue conflict
        cue_clean_feats = _extract_features_batched(backbone, cue_content_images)
        cue_transformed_feats = _extract_features_batched(backbone, cue_images)

        # Cosine stability
        stability_results[name] = {
            "grayscale": fs.cosine_stability(clean_feats, condition_feats["grayscale"]),
            "patch_shuffle": fs.cosine_stability(clean_feats, condition_feats["patch_shuffle"]),
            "translation": fs.cosine_stability(clean_feats, condition_feats["translation"]),
            "cue_conflict": fs.cosine_stability(cue_clean_feats, cue_transformed_feats),
        }
        ...
        for intervention in tqdm(
            ...
            combined = torch.cat([clean_feats, transformed], dim=0).numpy()
            ...
            combined_labels = np.concatenate([test_labels, test_labels])
            ...
            rep.plot_projection(
                ...
                save_path=os.path.join(fig_dir, f"{name}_{intervention}_{method}.png"),
            )

        # t-SNE / UMAP for cue conflicts
        combined_cc = torch.cat([cue_clean_feats, cue_transformed_feats], dim=0).numpy()
        ...
        cc_labels = np.array([m["content_class"] for m in cue_meta])

        rep.plot_projection(
            coords_cc,
            np.concatenate([cc_labels, cc_labels]),
            ["clean"] * len(cue_meta) + ["cue_conflict"] * len(cue_meta),
            title=f"{name}: clean vs cue_conflict ({method})",
            save_path=os.path.join(fig_dir, f"{name}_cue_conflict_{method}.png"),
        )

    output_path = os.path.join(RESULTS_DIR, "representation_stability_metrics.json")
